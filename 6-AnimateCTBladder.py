...
# loaded_X0 = torch.tensor(torch.load(object_name+"/"+training_name+"-training" +"/"+str(args[2])+"-sim_X0", map_location=torch.device(device))).float()
...
fname = "ExampleSetupScripts/Bladder/HipBonesCloud.obj"
hip, _, _, _, _, _ = igl.read_obj(fname)
ps_hipcloud = ps.register_point_cloud("hip bones", hip*10, enabled=True)
