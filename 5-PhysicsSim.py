...
def penalty(X0, x0_flat, B, J, z):
    ...
    exec(scene["object"][0]["move_bc"], globals(), l_dict)
...
def line_search(func, x, direction, gradient, alpha=0.5, beta=0.5):
    ...
    for _ in range(int(scene["ls_iters"])):
        x_new = x + t * direction
        f_new = func(x_new)
        f = func(x)
...
def simulate(np_X, Faces, np_YMs, WW, Phi, Handles):
    ...
    l_dict = {"X0": X0}
    exec(scene["object"][0]["set_moving_bc"], globals(), l_dict)
    ...
    l_dict = {"X0": X0}
    exec(scene["object"][0]["set_fixed_bc"], globals(), l_dict)
    indices = l_dict["indices"]
    indices = (3*indices).repeat_interleave(3)
    indices[1::3] += 1
    indices[2::3] += 2; 
    ...
    for step in range(int(scene["steps"])):
        ...
        for iter in range(int(scene["newton_iters"])):
...
random_batch_indices = np.random.randint(0, np_object["ObjectSamplePts"].shape[0], size=int(scene["sim_batch_size"])) 
